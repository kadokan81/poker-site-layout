    ...
    if (animateItems.length > 0) {
        ...
        function animeOnScroll() {
            for (let index = 0; index < animateItems.length; index++) {
                ...
                const animeItemOfSet = offSet(animItem).top;
                const animeStart = 4;
                let animeItemPoint = window.innerHeight - animItemHight / animeStart;
                if (animItemHight > window.innerHeight) animeItemPoint = window.innerHeight - window.innerHeight / animeStart;
                if (scrollY > animeItemOfSet - animeItemPoint && scrollY < animeItemOfSet + animItemHight) animItem.classList.add("_active"); else if (!animItem.classList.contains("_anime-no-hide")) animItem.classList.remove("_active");
            }
        }
        animeOnScroll();
        function offSet(el) {
            const rect = el.getBoundingClientRect();
            const scrollLeftEl = window.pageXOffset || document.documentElement.scrollLeft;
            const scrollTopEl = window.pageYOffset || document.documentElement.scrollTop;
            return {
                top: rect.top + scrollTopEl,
                left: rect.left + scrollLeftEl
            };
        }
    }
    window["FLS"] = true;
